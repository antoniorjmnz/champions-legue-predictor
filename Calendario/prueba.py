...
def imprimir_rivales(rivales, equipos_dict):
    ...
    for eq in sorted(rivales.keys()):
        ...
        print(
            f"{eq} ({pais}) | Total rivales: {len(rivales_list)} -> [{paises_str}] -> {', '.join(rivales_list)}"
        )
    print("\n=============================\n")
...
def generar_rivales(equipos_dict, max_iter=10000):
    ...
    # Llamada a la metaheurística (sin imprimir internamente)
    rivales = generar_rivales_metaheuristica(
        lista_equipos, equipos_dict, bombos, iteraciones=50000, max_mismo_pais=2
    )

    if rivales:
        print("Rivales generados correctamente.\n")
        # Imprime rivales en un único bloque
